...
func TestGroupCountsInclusive(t *testing.T) {
	...
	for i, c := range cases {
		actual := GroupCountsInclusive(c.input)
...
func TestGroupCountsExclusive(t *testing.T) {
	...
	for i, c := range cases {
		actual := GroupCountsExclusive(c.input)
		if actual != c.expected {
			t.Errorf("[case %d] expected: %d, actual: %d", i, c.expected, actual)
		}
	}
